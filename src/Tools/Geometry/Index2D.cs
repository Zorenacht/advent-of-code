...
public readonly struct Index2D(int row, int col) : IEquatable<Index2D>
{
    ...
    public IEnumerable<(Index2D Index, int Distance)> WithinManhattanDistance(int distance)
    {
        for (int i = 0; i <= distance; ++i)
        {
            for (int j = -distance + i; j <= distance - i; ++j)
            {
                ...
                yield return (new Index2D(Row + i, Col + j), dist);
                if (i != 0) yield return (new Index2D(Row - i, Col + j), dist);
            }
        }
    }
}
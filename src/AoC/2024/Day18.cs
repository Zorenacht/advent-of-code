...
public sealed class Day18 : Day
{
    ...
    {
        var lines = Input;
        var maze = InitMaze(71,71, 1024, lines);
        for (int i = 1024; i < lines.Length; ++i)
        {
            var ints = lines[i].Ints();
            maze.UpdateAt(ints[0], ints[1], '#');
            if (ShortestPath(Index2D.O, new Index2D(maze.RowLength - 1, maze.ColLength - 1), maze) != -1) continue;
            return lines[i];
        }
        return string.Empty;
    }
    
    public Maze InitMaze(int rows, int cols, int take,  string[] blockades)
    {
